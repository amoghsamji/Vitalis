  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader
        ...
        actions={
          <Link href="/doctor/workflows" className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
      ...
      <Card className="flex flex-wrap items-end gap-3">
        <div className="min-w-[200px] flex-1">
          ...
          <Input id="workflowName" className="mt-1.5" value={name} onChange={(e) => setName(e.target.value)} />
        </div>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="h-[600px] flex-1 rounded-lg border border-border bg-muted/30">
          <ReactFlow
            nodes={nodes}
            edges={edges}
        ...
        <Card>
          ...
          {runs.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {runs.map((run) => (
                <div key={run.runId} className="rounded-md border border-border p-3 text-sm">
                  <p className="text-xs text-muted-foreground">{new Date(run.completedAt).toLocaleString()}</p>
