            ...
            <Card key={wf.id} className="flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <Link href={`/doctor/workflows/builder?id=${wf.id}`} className="font-medium hover:text-brand-700">
                    {wf.name}
                  </Link>
                  <Badge variant={wf.enabled ? "success" : "neutral"}>{wf.enabled ? "Enabled" : "Disabled"}</Badge>
                </div>
                <p className="text-sm text-slate-500">{TRIGGER_LABELS[wf.triggerType] ?? wf.triggerType}</p>
              </div>
              <div className="flex items-center gap-2">
